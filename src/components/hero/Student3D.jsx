...
const passportMat = new THREE.MeshPhysicalMaterial({
  color: "#B3232E",
  roughness: 0.4,
  clearcoat: 0.3,
  clearcoatRoughness: 0.5,
...
const StudentRig = (props) => {
  ...
  useFrame((state, delta) => {
    ...
    if (reduced) {
      ...
      jawRef.current.rotation.x = THREE.MathUtils.damp(
    ...
    lidLRef.current.rotation.x = THREE.MathUtils.damp(
      lidLRef.current.rotation.x,
      lidRot,
      60,
      dt,
    );
    lidRRef.current.rotation.x = THREE.MathUtils.damp(
      lidRRef.current.rotation.x,
      lidRot,
      60,
      dt,
    );
    ...
    if (talkT.current < talkMs.current) {
      const idx = Math.min(
        Math.floor((talkT.current / talkMs.current) * env.current.length),
        env.current.length - 1,
      );
    ...
    smileAmt.current = THREE.MathUtils.damp(
      smileAmt.current,
      happy ? 1 : 0.75,
      5,
      dt,
    ...
    headTrackRef.current.rotation.z = THREE.MathUtils.damp(
      headTrackRef.current.rotation.z,
      0.02,
      4,
      dt,
    );
    ...
    irisLRef.current.position.x = THREE.MathUtils.damp(
      irisLRef.current.position.x,
      state.pointer.x * 0.02,
      10,
      dt,
    );
    irisLRef.current.position.y = THREE.MathUtils.damp(
      irisLRef.current.position.y,
      state.pointer.y * 0.014,
      10,
      dt,
    );
    irisRRef.current.position.x = THREE.MathUtils.damp(
      irisRRef.current.position.x,
      state.pointer.x * 0.02,
      10,
      dt,
    );
    irisRRef.current.position.y = THREE.MathUtils.damp(
      irisRRef.current.position.y,
      state.pointer.y * 0.014,
      10,
      dt,
    );
        ...
        <group position={[0, 0.78, 0]}>
          <group ref={headTrackRef}>
            <group ref={headBobRef}>
              ...
              {/* ---- Stylized Pixar/Premium Face (No creepy realistic features) ---- */}
